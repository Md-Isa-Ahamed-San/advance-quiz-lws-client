...
const QuizResult = ({ quizSetId }) => {
  ...
  const { totalQuestions, correctCount, wrongAnswers, totalGotMark } = stats;
  const title = quizResultData?.data?.quiz?.title || "Unknown Title";
  const description = quizResultData?.data?.quiz?.description || "No Description";

  return (
      <div
          className="max-h-screen overflow-hidden hidden lg:flex lg:w-1/2 bg-primary flex-col justify-center p-12 relative">
        <div>
          <div className="text-white">
            ...
            <div className="my-6 flex items-center">
              <div className="w-1/2">
                ...
                <NavLink to={`/leaderboard/${quizSetId}`}
